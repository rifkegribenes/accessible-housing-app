  ...
  it("POST creates a new user", () => {
    return users
      ...
      .then(result => {
        ...
        assert.deepEqual(result[0].user_type, user_type);
      ...
      .then(([result]) => {
        ...
        assert.equal(result.user_type, user_type);
        userId = result.id;
      });
  });
  ...
  describe("secured routes", () => {
    ...
    it("PUT updates a user", () => {
      ...
      return users.updateUser(userId, updates).then(results => {
        ...
        assert.equal(results[0].user_type, user_type);
    ...
    it("GET gets all users", () => {
      return users.getUsers().then(results => {
        ...
        assert.include(arrayOfKeys("user_type"), user_type);
        ...
        assert.deepEqual(results[0].user_type, user_type);
      });
    });
    ...
    it("GET gets one user by id", () => {
      return users.getUserById(userId).then(returnedUser => {
        ...
        assert.equal(returnedUser.user_type, user_type);
      });
    });
